...
import { ApiChat, API_BASE } from '../lib/api';
...
export const ChatProvider: React.FC<ChatProviderProps> = ({ children, userId }) => {
  ...
  useEffect(() => {
    ...
    const resolveSocketBaseUrl = () => {
      const explicitSocket = (import.meta as any).env?.VITE_SOCKET_URL as string | undefined;
      if (explicitSocket && /^https?:\/\//i.test(explicitSocket)) {
        return explicitSocket.replace(/\/$/, '');
      }

      // Prefer API_BASE when it is an absolute URL
      ...
      // Production fallback: same-origin (useful if served behind same domain/proxy)
